...
async def upload_image(file: UploadFile = File(...), x_api_key: str = Header(...)):
    ...
    # ✅ Your original system prompt restored exactly
    system_prompt = (
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": system_prompt},
            {
                "role": "user",
                "content": f"""
Please analyze this image and return a detailed diagnostic report using the following structure:
...
async def follow_up(question: str = Form(...), session_id: str = Form(...), x_api_key: str = Header(...)):
    ...
    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are a board-certified radiologist providing clinical explanations."},
